...
function spellNumberUzbek(numberStr) {
  ...
  const strVal = String(numberStr);
  let num = parseInt(strVal.replace(/[^\d\-]/g, ""), 10);
...
export async function speakText(text, language = "uz") {
  ...
  // O'zbek tilidan boshqa barcha tillar brauzerning ichki TTSidan foydalanadi
  if (ttsLang !== "uz") {
    fallbackSpeech(ttsText, ttsLang);
    return;
  }

  // Faqat o'zbek tili uchun maxsus Gemini TTS ishlatiladi
  resetTtsPlayback();
  window.speechSynthesis.cancel();
...
export async function startRecording() {
  ...
  pcmData = []; // ALBATTA HAR BIR YOZUV BOSHLANISHIDA TOZALASH SHART
  ...
  try {
    ...
    try {
      await state.inputAudioContext.audioWorklet.addModule(
        "/recorder-worklet.js?v=" + Date.now(),
      );
...
async function onRecordingStop() {
  ...
  try {
    ...
    if (data.error) {
       if (assistantTextElement) assistantTextElement.innerText = data.error;
       state.isRecording = false;
       const btnVoice = document.getElementById("btn-voice");
       if (btnVoice) btnVoice.classList.remove("active-recording");
       return;
    ...
    if (assistantTextElement)
      assistantTextElement.innerText = "Hozirda ovozli xizmatda uzilish bor. Iltimos, qaytadan urinib ko'ring.";
  }
  state.isRecording = false;
  const btnVoice = document.getElementById("btn-voice");
  if (btnVoice) btnVoice.classList.remove("active-recording");
