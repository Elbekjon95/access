...
export function showEarthRouteActionNote(text) {
  const container = document.getElementById("earth-system-note");
  if (container) {
    container.innerHTML = `<div style="border-top: 1px solid rgba(0,255,160,0.3); margin-top: 15px; padding-top: 10px; font-size: 0.9em; color: #00ffaa; font-family: 'Courier New', monospace; line-height: 1.4; white-space: pre-wrap;">
      <span style="opacity: 0.7; font-weight: bold;">>> SYSTEM:</span> ${text}
    </div>`;
  }
}
...
export async function renderEarthRouteActions(destinationCode, replyText) {
  ...
  try {
    ...
    const noteArea = document.getElementById("earth-system-note");
    ...
    withCounters.forEach(({ flight, counterRange }) => {
      ...
      btn.type = "button";
      btn.innerHTML = `Ro'yxatdan o'tish <span class="counter" style="background: #00ffaa; color: #000; padding: 2px 6px; border-radius: 4px; font-weight: bold; margin: 0 4px;">${counterRange}</span> stoykalarida ochiq.`;
      btn.onclick = () =>
