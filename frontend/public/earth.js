...
async function loadFlightModelTemplate() {
  ...
  flightModelLoadPromise = (async () => {
    ...
    // 1-bosqich: Mahalliy va tashqi manbalarni tekshirish
    const modelCandidates = [];
    bases.forEach(b => {
      modelCandidates.push(joinBasePath(b, "models/airplane/scene.gltf"));
    });
    
    // Zaxira: jsDelivr CDN (ishonchli manba)
    modelCandidates.push("https://cdn.jsdelivr.net/gh/KhronosGroup/glTF-Sample-Models@master/2.0/CesiumAir/glTF-Binary/CesiumAir.glb");
    modelCandidates.push("https://cdn.jsdelivr.net/gh/KhronosGroup/glTF-Sample-Models@master/2.0/CesiumAir/glTF/CesiumAir.gltf");

    for (const url of modelCandidates) {
      try {
        console.log(`[EARTH] Trying to load model from: ${url}`);
        
        // Fayl borligini va HTML emalligini tekshirish
        const check = await fetch(url, { method: 'HEAD' });
        if (!check.ok || check.headers.get('content-type').includes('text/html')) {
          console.warn(`[EARTH] Resource at ${url} is not a valid model file.`);
          continue;
        }
        ...
        console.log(`[EARTH] Flight model loaded successfully from: ${url}`);
        return flightModelTemplate;
      } catch (err) {
        lastError = err;
        console.warn(`[EARTH] Failed to load model from ${url}:`, err.message);
      }
    }
...
function createFlightPlane(color = 0xffffff) {
  ...
  group.scale.setScalar(FLIGHT_PLANE_SCALE * 1.5); // 1.5x larger fallback
  return group;
}
...
function updateFlightAnimation(deltaSec) {
  ...
  const lift = p.clone().normalize().multiplyScalar(0.025); // Co'tarildi (0.0012 dan 0.025 ga)
